#!/usr/bin/env python3
"""
linkedin_image_processor.py - Handles image selection and uploading to ImgBB for LinkedIn posts
...
logging.basicConfig(
    ...
    filename="linkedin_image_processor.log",
)
logger = logging.getLogger(__name__)

# Directory configuration
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
IMAGE_DIR = os.path.join(BASE_DIR, "linkedin_images")  # Separate directory for LinkedIn
USED_IMAGE_DIR = os.path.join(BASE_DIR, "linkedin_images", "used")
CAPTIONS_FILE = os.path.join(BASE_DIR, "linkedin_image_captions.json")
URL_CACHE_FILE = os.path.join(BASE_DIR, "linkedin_image_url_cache.json")
...
def get_image_caption(image_name, captions=None):
    ...
    # Add LinkedIn-relevant text to default captions
    prefixes = [
    ...
    # Randomly choose a prefix if caption doesn't already sound professional
    if not any(
        word in default_caption.lower()
        for word in ["professional", "business", "strategy", "leadership"]
    ):
...
def select_random_image():
    ...
    try:
        ...
        if os.path.exists(IMAGE_DIR) and os.listdir(IMAGE_DIR):
            ...
            # Fallback to Twitter images if LinkedIn directory is empty
...
def move_used_image(image_path):
    ...
    try:
        ...
        if "linkedin_images" in image_path:
...
if __name__ == "__main__":
    ...
    print("=== LinkedIn Image Processor Test ===")
